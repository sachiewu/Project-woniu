...
!function ($) {
    ...
    var price = $('.sellprice');
    var btn = $('.carBtns');
    var numvalue = $('.pro_num');
    var alertbox = $('.alertbox');
    var close = $('.alertbox span');
    var btn2 = $('.btn2');
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function success(objdata) {
    ...
    btn.on('click', function () {
        ...
        if (arrsid.indexOf(sid) === -1) {
            ...
            //存在追加数量值  查找上一次的数量
            var sum = parseInt(arrnum[arrsid.indexOf(sid)]) + parseInt(numvalue.value);
            arrnum[arrsid.indexOf(sid)] = sum;
            setcookie('cookienum', arrnum.toString(), 30);
        }
